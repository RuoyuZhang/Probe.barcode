...
Script from Brendan Kohrn and Scott Kennedy(1)
...
optional arguments:
  ...
  --cutoff CUTOFF       Percentage of nucleotides at a given position in a
                        ...
                        to be called at that position. [0.5]
...
#change DNA to its complement sequence
def complement(s):
    basecomplement = {'A': 'T', 'C': 'G', 'G': 'C', 'T': 'A' , 'N': 'N', 'a':'t', 't':'a', 'g': 'c', 'c': 'g', 'n':'n'}
    letters = list(s)
    letters = [basecomplement[base] for base in letters]
    return ''.join(letters)

#revcom(DNAsequence)
#change DNA to its reverse complement sequence
def revcom(s):
...
def tagStats(tagCountsFile,filename):
...
def main():
    ...
    parser.add_option("--infile", action="store", dest="infile", help="input BAM file")
    parser.add_option("--tagfile",  action="store",  dest="tagfile", help="output tagcounts file",  default='sys.stdout')
    parser.add_option("--outfile",  action="store", dest="outfile", help="output BAM file")
    parser.add_option("--rep_filt", action="store",  type=int, dest='rep_filt', help="Remove tags with homomeric runs of nucleotides of length x. [9]", default=9 )
    parser.add_option('--minmem', type=int, default=3, dest='minmem', help="Minimum number of reads allowed to comprise a consensus. [3] ")
    parser.add_option('--maxmem', type=int, default=1000, dest='maxmem', help="Maximum number of reads allowed to comprise a consensus. [1000]")
    parser.add_option('--cutoff', type=float, default=.51, dest='cutoff', help="Percentage of nucleotides at a given position in a read that must be identical in order for a consensus to be called at that position. [0.7]")
    parser.add_option('--Ncutoff', type=float, default=1, dest='Ncutoff', help="With --filt 'n', maximum fraction of Ns allowed in a consensus [1.0]")
    parser.add_option('--readlength', type=int, default=84, dest='read_length', help="Length of the input read that is being used. [80]")
    parser.add_option('--read_type', type=str, action="store", dest='read_type', default="d", help="A string specifying which types of read to consider.  Read types: n: Neither read 1 or read 2 mapped.  m: Either read 1 or read 2 mapped, but not both.  p: Both read 1 and read 2 mapped, not a propper pair.  d: Both read 1 and read 2 mapped, propper pair.  s: Single ended reads\n\t\t['d']")
    parser.add_option('--isize', type = int, default=-1, dest='isize', help="maximum distance between read pairs")
    parser.add_option('--read_out', type = int, default = 1000000, dest = 'rOut', help = 'How often you want to be told what the program is doing. [1000000]')
    parser.add_option('--filt', type=str, default='n', dest='filt', help="A string indicating which filters should be implemented.  Filters: s: Softclipping filter.  o: Overlap filter.  n: N filter.  ['osn']")
    ...
    errname = o.outfile.replace(".bam",".errRate")
    tagstatname = o.tagfile+'stat'
    errRatefile = open(errname, 'w')
    outR1 = gzip.open(o.outfile.replace(".bam",".R1.fq.gz"),"wb")
    outR2 = gzip.open(o.outfile.replace(".bam",".R2.fq.gz"),"wb")
    if o.read_type == 'd':
            ...
            if int( readWin[winPos%2].flag ) in goodFlag and overlap==False and softClip==False: 
                if ('A'*o.rep_filt in tag) or ('C'*o.rep_filt in tag) or ('G'*o.rep_filt in tag) or ('T'*o.rep_filt in tag): 
                    ...
                    if str(readWin[winPos%2].cigar) not in readDict[tag][6]:
                        # in this array, 0 is number of reads with this cigar, 1 is the cigar, 2 to n is a array, 0 is read seq, 1 is quality
                        readDict[tag][6][str(readWin[winPos%2].cigar)]=[0,readWin[winPos%2].cigar]
                    
                    readDict[tag][6][str(readWin[winPos%2].cigar)].append([readWin[winPos%2].seq,readWin[winPos%2].qqual])
            ...
            for dictTag in readDict.keys(): # Extract sequences to send to the consensus maker
                ...
                if cigComp[maxCig] >= o.minmem:
                    if cigComp[maxCig] <= o.maxmem:
                        ConMade += 1
                        consensus, consensusQ, fam_size, errRate = consensusMaker(readDict[dictTag][6][maxCig][2:], o.cutoff, o.read_length, readDict[dictTag][0] )
                    else:
                        ConMade += 1
                        consensus, consensusQ, fam_size, errRate = consensusMaker(random.sample(readDict[dictTag][6][maxCig][2:], o.maxmem), o.cutoff, o.read_length,readDict[dictTag][0])

                    errRatefile.write('%s\t%s\t%s\t%s\t%s\t%s\n' %(dictTag,str(errRate[0][0]),str(errRate[0][1]),str(errRate[0][2]),str(errRate[0][3]), str(errRate[1])))
                    ...
                    for cigStr in readDict[dictTag][6].keys():
                        if cigStr != maxCig:
                            for n in xrange(2, len(readDict[dictTag][6][cigStr][2:])):
                                ...
                                a.seq = readDict[dictTag][6][cigStr][n][0]
...
#                            outR1.write('@%s\n%s\n+\n%s\n' %(altTag,a.seq,a.qual))
                            ...
                            if a.is_read1 == True:
                                ...
                                if a.is_reverse:
                                    [r1seq,r1q,r2seq,r2q] = [revcom(a.seq),a.qual[::-1],b.seq,b.qual]
                                else:
                                    [r1seq,r1q,r2seq,r2q] = [a.seq,a.qual,revcom(b.seq),b.qual[::-1]]
...
#                                outR2.write('@%s\n%s\n+\n%s\n' %(altTag,a.seq,a.qual))
                                ...
                                if a.is_reverse:
                                    [r2seq,r2q,r1seq,r1q] = [revcom(a.seq),a.qual[::-1],b.seq,b.qual]
                                else:
                                    [r2seq,r2q,r1seq,r1q] = [a.seq,a.qual,revcom(b.seq),b.qual[::-1]]
    ...
    tagFile.close()
    tagStats(o.tagfile,tagstatname)
